...
const MeetingSchedulePreview = observer(() => {
  ...
  const [lastMeetingScheduleName, setLastMeetingScheduleName] = useState<
    string | null
  >(null);
  const [lastMeetingScheduleId, setLastMeetingScheduleId] = useState<
    string | null
  >(null);
  ...
  const getData = async () => {
    ...
    if (isAdmin && currentRole === 2) {
      meetingScheduleData = await listMeetingScheduleInClass(
