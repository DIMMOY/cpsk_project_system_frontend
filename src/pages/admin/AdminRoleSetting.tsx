    ...
    <AdminCommonPreviewContainer>
      <Box sx={{display: 'flex', flexDirection: 'column', width: '100%'}}>
        <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center'}}>
            <Typography
                sx={{ fontSize: 45, fontWeight: 600, color: theme.color.text.primary, marginRight: '1rem' }}
            >
            แอดมิน
            </Typography>
            {
                !isEditAdmin ?
                <>
                    <ActivateButton sx={{width: '5rem', marginRight: '1rem'}} onClick={() => setOpenAdminModal(true)}>
                        เพิ่ม
                    </ActivateButton>
                    <UserAddRoleModal
                        open={openAdminModal}
            ...
            }
            {
                isEditAdmin ?
                <>
                    <ActivateButton sx={{width: '5rem'}} onClick={submitToSubmitAdmin}>
                        บันทึก
                    </ActivateButton>
                    <CancelButton sx={{marginLeft:'1rem', width: '5rem'}} onClick={cancelToSubmitAdmin}>
                        ยกเลิก
                    </CancelButton>
                </> : <></>
            }
                ...
                admins.map((c, index) => (
                    ...
                    >
                        ...
                        <Typography sx={{ color: theme.color.text.secondary }}>
                            {c.userId.displayName && c.userId.displayName.length ? c.userId.displayName : '...'}
                        </Typography>
                        <Typography sx={{ color: theme.color.text.secondary }}>
                            {c.userId.email}
                        </Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginTop: '2rem'}}>
            <Typography
                sx={{ fontSize: 45, fontWeight: 600, color: theme.color.text.primary, marginRight: isBigScreen ? '1rem' : '0rem' }}
            >
                อาจารย์ที่ปรึกษา
            </Typography>
            {
                !isEditAdvisor ?
                <>
                     <ActivateButton sx={{width: '5rem', marginRight: '1rem'}} onClick={() => setOpenAdvisorModal(true)}>
                        เพิ่ม
                    </ActivateButton>
            ...
            }
            {
                isEditAdvisor ?
                <>
                    <ActivateButton sx={{width: '5rem'}} onClick={submitToSubmitAdvisor}>
                        บันทึก
                    </ActivateButton>
                    <CancelButton sx={{marginLeft:'1rem', width: '5rem'}} onClick={cancelToSubmitAdvisor}>
                        ยกเลิก
                    </CancelButton>
                </> : <></>
            }
