...
import { Box, IconButton, Button, Typography, Link as LinkMUI, Divider } from '@mui/material'
...
import { deleteDocumentFromStudent, getDocumentFromStorage, uploadDocumentFromStudent } from '../../utils/storage'
import applicationStore from '../../stores/applicationStore'
import { LoadingButton } from '@mui/lab'
import { cancelSendDocument, getSendDocumentInClass, sendDocument } from '../../utils/document'
      ...
      <CommonPreviewContainer>
        ...
        <Box
          ...
          >
            ...
            {
              description.length ? 
                <Divider sx={{ margin: "0 2rem 0 2rem", color: theme.color.background.tertiary}} /> :
                <></>
            }
            {
              description.length ? 
              ...
              {files.map((file, index) => (
                  <Box key={`file-${index}`} 
                    ...
                    {(!status ? file.type.startsWith('image/') : metaDatas[index].contentType?.startsWith('image/')) ? (
                      ...
                      <img 
                        style={{ width: '8rem', height: '8rem', borderRadius: "10px" }} 
                        src={(!status ? file.type.startsWith('application/pdf') : metaDatas[index].contentType?.startsWith('application/pdf')) ? PDF : Others} alt="" 
                      />
                    )}
                    <Typography>
                      <LinkMUI 
              ...
              { isStudent && !status ?
                <Box
                  ...
                  <IconButton sx={{width: '8rem', height: '8rem'}}
                    aria-label="upload" 
                    component="label">
                    <input hidden accept="*" type="file" multiple onChange={handleAddFile}/>
