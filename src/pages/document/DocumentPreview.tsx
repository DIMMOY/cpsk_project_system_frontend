...
const DocumentPreview = observer(() => {
  ...
  const getData = async () => {
    ...
    if (isAdmin && currentRole === 2) {
      documentData = await listDocumentInClass(
  ...
  const handleCancelSubmit = async () => {
    const result = await disabeDocumentInClass(
      classId,
              ...
              >
                ...
                >
                  {isAdmin && currentRole === 2
                    ? c.statusInClass
                      ? (isBigScreen ? `เปิดใช้งานวันที่ ` : "") +
                        `${moment(c.startDate).format("DD/MM/YYYY HH:mm")}`
