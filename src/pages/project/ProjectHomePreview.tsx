...
const ProjectHomePreview = observer((props: { isStudent: boolean, isCommittee: boolean }) => {
    ...
    return (
      <ProjectPreviewContainer>
        <ProjectPreviewDetail>
          ...
          <Typography sx={{fontSize: 30, fontWeight: 500, color: theme.color.text.secondary}}>{nameEN}</Typography>
          <Typography sx={{fontSize: 30, fontWeight: 500, color: theme.color.text.secondary}}>{description}</Typography>
        </ProjectPreviewDetail>
        <Box sx={{textAlign: 'center'}}>
          <Link to = {currentRole === 0 ? "/document" : `${cuurentPathName}/document`} style={{ textDecoration: 'none' }}>
          ...
          {!isCommittee && (
            <Link to = {currentRole === 0 ? "/meeting-schedule" : `${cuurentPathName}/meeting-schedule`} style={{ textDecoration: 'none' }}>
              <ProjectPreviewButton isBigScreen={isBigScreen}
                onClick={scrollTop}
              >
