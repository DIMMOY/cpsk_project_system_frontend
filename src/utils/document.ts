...
export const createDocument = async (reqBody: any) => {
  try {
    await refreshToken();
    await axios.post(
      `${process.env.REACT_APP_API_BASE_URL_CLIENT as string}/document`,
      reqBody
    );
    return {
      statusCode: 201,
...
export const setDateDocument = async (reqBody: any) => {
  try {
    ...
    const url = `${process.env.REACT_APP_API_BASE_URL_CLIENT as string}/class/${
      reqBody.classId
    }/document/${reqBody.documentId}/date`;
...
export const listSendDocumentInClass = async (
  reqQuery: any,
  classId: string,
  projectId: string
) => {
  try {
    ...
    const url = `${
      process.env.REACT_APP_API_BASE_URL_CLIENT as string
    }/class/${classId}/project/${projectId}/document`;
    const resAxios = await axios.get(url, { params: reqQuery });
    return {
      data: resAxios.data.data,
    };
...
export const getSendDocumentInClass = async (
  classId: string,
  projectId: string,
  documentId: string
) => {
  try {
    ...
    const url = `${
      process.env.REACT_APP_API_BASE_URL_CLIENT as string
    }/class/${classId}/project/${projectId}/document/${documentId}/detail`;
...
export const cancelSendDocument = async (
  projectId: string,
  documentId: string
) => {
  try {
...
export const disabeDocumentInClass = async (
  classId: string,
  documentId: string
) => {
  try {
    ...
    const url = `${
      process.env.REACT_APP_API_BASE_URL_CLIENT as string
    }/class/${classId}/document/${documentId}/date/status`;
