...
const AddCommitteeToProject = ({
  ...
  const handleSubmit = async () => {
    ...
    const deleteProjectInGroup = projectIdInGroup.filter(
      (id: string) => !checked.includes(id)
    );
    const createProjectInGroup = checked.filter(
      (id: string) => !projectIdInGroup.includes(id)
    );

    const res = await createMatchCommitteeHasGroupToProject(
      classId,
      committeeId,
      matchCommitteeHasGroup._id,
      createProjectInGroup,
      deleteProjectInGroup
    );
            ...
            {projects.map((data, index) => (
              ...
              >
                ...
                <TableCell>
                  ...
                  !data.advisor
                    .map((a: { _id: any }) => a._id)
                    .filter((id: any) => advisorIdInGroup.includes(id))
                    .length ? (
                    <Checkbox
                      sx={{
                        padding: 0,
